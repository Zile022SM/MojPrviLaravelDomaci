...
<section class="bg-white dark:bg-gray-900">
  ...
  <div class="py-8 lg:py-16 px-4 mx-auto max-w-screen-md">
      <h2 class="mb-4 text-4xl tracking-tight font-extrabold text-center text-gray-900 dark:text-white">Edit {{$contact->email}}</h2>
     
      <form method="POST" action="{{route('update-contact', $contact->id)}}" class="space-y-8">
      @csrf
      @method('POST')
          <div>
              <label for="email" class=" mb-2 text-sm font-medium text-gray-900 dark:text-gray-300">Your email</label>
              <input id="email" name="email" value="{{old('email', $contact->email)}}" class="shadow-sm bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-500 focus:border-primary-500  w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500 dark:shadow-sm-light" placeholder="name@flowbite.com">
              @error('email')
